...
class MachineLogger:
    ...
    def log_transition(self, current_state, symbol, next_state, write_symbol, direction):
        """Log a state transition"""
        if not self.enabled:
            return
        dir_str = str(direction) if direction else 'None'
        self.logger.info(f"{current_state} ({symbol}) -> {next_state} [write: {write_symbol}, dir: {dir_str}]")
    
    def log_tape_state(self, tape, current_pos, current_state):
        """Log current tape state with pointer"""
        if not self.enabled:
            return
        self.logger.info(f"State: {current_state} | {self._format_tape(tape, current_pos)}")
